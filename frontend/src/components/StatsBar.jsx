...
export default function StatsBar() {
  const { stats, filters, setFilters, setIsResilienceModalOpen } = useDisaster();
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
      ...
      <div 
        ...
        className="cursor-pointer bg-[#111827]/80 hover:bg-emerald-950/30 border border-emerald-900/40 rounded-xl p-3 flex flex-col justify-between transition-all shadow-md"
      >
        <div className="flex items-center justify-between text-xs text-emerald-300">
          <span className="font-medium">Cluster Resilience</span>
          <span className="p-1 rounded-md bg-emerald-950/60 text-emerald-400">
        ...
        <div className="mt-2 flex items-baseline justify-between">
          ...
          <span className="text-[10px] text-slate-400 font-medium">Failover Safe</span>
        </div>
      </div>
    </div>
  );
